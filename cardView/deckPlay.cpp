...
void DeckPlay::start(sessionStr newSession)
{
    ...
    if(currectSession.core.mode == CompletlyRandomised) {
        ...
        mode->setup(this, ui);
    } else if (currectSession.core.mode == SM2) {
        sm2Mode* mode = new sm2Mode(this);
        sm2ModeInstance = mode;
        connect(this, &DeckPlay::saveData, mode, &sm2Mode::saveData);
        mode->setup(this, ui);
    }
...
void DeckPlay::exitIt() {
    ...
    if (sm2ModeInstance) {
        sm2ModeInstance->saveData();
        sm2ModeInstance->showSessionSummary();
    }
...
void DeckPlay::receiveDeckCall(QString call) {
    ...
    if (sm2ModeInstance && (call == "cram" || call == "filter" || call == "suspend" || call == "bury" || call == "flag" || call == "minimal")) {
        sm2ModeInstance->receiveCall(call);
    }
}
