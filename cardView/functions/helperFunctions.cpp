...
{
    if (!hasClozeDeletions(*frontCard)) {
        return;
    }
    ...
    frontResult.replace(reHint, "<span class=\"cloze-hint\">[\\1]</span>");
    frontResult.replace(reSimple, "<span class=\"cloze\">[...]</span>");

    QString backResult = original;
    backResult.replace(reHint, "<b>\\2</b>");
    backResult.replace(reSimple, "<b>\\1</b>");
